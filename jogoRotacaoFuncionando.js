...
function rotacionarPeca() {
    ...
    let numLinhas = pecaAtual.length;
    let numColunas = pecaAtual[0].length;
    for(let coluna = 0; coluna < numColunas; coluna++) {
        novaPeca[coluna] = [];
        for(let linha = numLinhas - 1; linha >= 0; linha--) {
            novaPeca[coluna].push(pecaAtual[linha][coluna]);
        }
    }

    for(let linha = 0; linha < numLinhas; linha++) {
        for(let coluna = 0; coluna < numColunas; coluna++) {
            if(pecaAtual[linha][coluna] != 0) {
                matriz[yPecaAtual + linha][xPecaAtual + coluna] = 0;
            }
    ...
    for(let linha = 0; linha < numLinhas; linha++) {
        for(let coluna = 0; coluna < numColunas; coluna++) {
            if(pecaAtual[linha][coluna] != 0) {
                matriz[yPecaAtual + linha][xPecaAtual + coluna] = pecaAtual[linha][coluna];
            }
        }
    }
    desenharIndicador();
